  ...
  const Input = ({ label, type = "text", value, onChange }: any) => (
    <div className="flex flex-col">
      ...
      <input
        ...
        value={value}
        onChange={onChange}
        className="w-full rounded-xl border border-gray-300 px-3 py-2 shadow-sm focus:border-green-500 focus:ring-2 focus:ring-green-200 transition"
      />
    </div>
  );
        ...
        {step === 1 && (
          <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <Input label="Address Line 1" value={form.addressLine1} onChange={(e:any)=>setForm({...form, addressLine1:e.target.value})}/>
            </div>
            <div className="md:col-span-2">
              <Input label="Address Line 2 (optional)" value={form.addressLine2} onChange={(e:any)=>setForm({...form, addressLine2:e.target.value})}/>
            </div>
        ...
        {step === 3 && (
          <div>
            <div className="mb-6">
              ...
              <p className="text-gray-500 text-sm mb-6">Add any supporting documents for this HR profile. All uploaded files will be listed below.</p>
              {/* Profile Image Upload */}
              <div className="flex items-center gap-6 mb-8">
                <div className="flex flex-col items-center">
                  <div className="w-20 h-20 bg-gray-100 rounded flex items-center justify-center mb-2 border border-dashed">
                  ...
                  <input type="file" accept="image/*" className="hidden" id="add-profile-upload" onChange={async e => {
                  ...
                  <label htmlFor="add-profile-upload" className="bg-gray-200 px-3 py-1 rounded text-sm cursor-pointer">Choose File</label>
                  <span className="text-xs text-gray-400 mt-1">Please upload square image, size less than 100KB</span>
                </div>
              </div>
              {/* Document Upload */}
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <input type="file" className="border rounded px-2 py-1" onChange={e => setDocFile(e.target.files?.[0] || null)} />
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setForm((f:any) => ({
                          ...
                          documents: [
                            ...f.documents,
                            { type: docType, url, uploadedAt: new Date().toISOString() },
                          ],
                        }));
                        setDocType("");
                        setDocFile(null);
                      ...
                      <div key={idx} className="flex flex-col items-center">
                        <div className="w-16 h-16 bg-gray-100 rounded flex items-center justify-center border mb-1">
                          {doc.url.endsWith('.pdf') ? (
                            <img src="/pdf-icon.png" alt="PDF" className="w-10 h-10" />
                          ) : (
                            <img src={doc.url} alt={doc.type} className="w-16 h-16 object-cover rounded" />
                          )}
                        </div>
                        <span className="text-xs mt-1">{doc.type.charAt(0).toUpperCase() + doc.type.slice(1)}</span>
                      </div>
                    ))
                  ) : (
